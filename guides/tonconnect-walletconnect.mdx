...
When using TON Connect with WalletConnect, `ton_proof` authentication works differently than with injected wallets. With injected wallets, the proof is returned automatically as part of the connect response items. With WalletConnect, `ton_proof` is handled via the [CAIP-222](https://chainagnostic.org/CAIPs/caip-222) authentication flow.
