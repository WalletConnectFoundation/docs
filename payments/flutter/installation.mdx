...
When calling `posClient.setTokens(tokens: tokens);` your are essentially setting which tokens your POS Client would support. However, our Blockchain API might not support some network, therefor, some tokens on that network. During  `init()` method our SDK will check the currently supported networks and filter out the unsupported tokens from your initial list based on the supported networks.
...
dependencies:
  reown_pos:
    git:
      ...
      ref: 769c0b0b975332c00a8f7fe19a0d63b18925d455
